...
#define LCF_TRAPVEC0_START	0x80008100  // App trap table (tranh vung bootloader)
...
#define RESET 0x80008020  // App entry point (bootloader chiem 0x80000020)
...
derivative tc27A
{
	...
	{
		mau = 8;
		size = 2M - 32k;  // [CUSTOM] bo qua 32KB dau (bootloader region)
		type = rom;
		map     cached (dest=bus:sri, dest_offset=0x80008000,           size=2M - 32k);
		map not_cached (dest=bus:sri, dest_offset=0xa0008000, reserved, size=2M - 32k);
	}
...
#			include "inttab0.lsl"
	...
	section_layout :vtc:linear
	{		
		group  reset (ordered, run_addr=0x80008020)  // App reset vector
		{
			select "*.start";
		}
		group  app_checksum (ordered, run_addr=0x8000803C)
        {
            select ".rodata.openblt_checksum";
        }
		...
		"__IF_CONST" := addressof(group:interface_const);  // [FIX] sua typo tu Bootloader LSL
